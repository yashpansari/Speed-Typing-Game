...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    lst = [i for i in paragraphs if select(i)]
    if k<len(lst):
        return lst[k]
    return ''
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    if len(typed_words)==0 or len(reference_words)==0:
        return 100.0*(len(typed_words)==len(reference_words))
    for i in range(min(len(typed_words),len(reference_words))):
        if typed_words[i]==reference_words[i]:
            acc += 1
    return 100*acc/len(typed_words)
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    return 12*len(typed)/elapsed #60/5=12
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    word = min(valid_words,key=lambda v : diff_function(v,typed_word,limit))
    if typed_word in valid_words or diff_function(word,typed_word,limit)>limit:
        return typed_word
    return word
    # END PROBLEM 5
...
def feline_flips(start, goal, limit):
    ...
    if limit==-1:
        return 0
    notequal = int(start[0] != goal[0])
    if min(len(start),len(goal))==1:
        return abs(len(start)-len(goal))+notequal
    return notequal+feline_flips(start[1:],goal[1:],limit-notequal)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    3
    """
    if limit==-1 or start==goal:  # Fill in the condition
        # BEGIN
        return 0
        # END
...
def fastest_words(match):
    ...
    t = get_times(match)
    w = get_words(match)
    result = [[] for c in range(len(t))]
    for i in range(len(t)):
        for j in range(len(w)):
            if w[j] and t[i][j] == min([t[x][j] for x in range(len(t))]):
                result[i].append(w[j])
                w[j] = ''
    return result
